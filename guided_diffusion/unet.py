...
class GenerativeDenoiser(th.nn.Module):
    ...
    def forward(self, y, sigma, labels=None, return_logvar=False, verbose=False, **kwargs):
        ...
        tmax = th.ones([y.shape[0], 1, 1, 1], device=y.device) * self.init_sigma
        
        # sampling loop
        xt = th.randn_like(y) * tmax
        x0 = self._forward(xt, tmax, labels, return_logvar=False)
        xt = scheduler.add_noise(x0, th.randn_like(x0), scheduler.timesteps[0])
        progress_bar = tqdm(scheduler.timesteps[:-1], disable=not verbose)
        for i in progress_bar:
            s, s_t = alphas[i] ** 0.5, (1 - alphas[i]) ** 0.5
            t = th.tensor([s_t / s], device=y.device).view(-1, 1, 1, 1)
